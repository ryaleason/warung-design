# Panduan Menjalankan Query di Supabase SQL Editor

File ini berisi query SQL lengkap untuk inisialisasi database Warung Desain di Supabase.

### Cara Menggunakan:
1. Buka dashboard Supabase project Anda (`https://supabase.com/dashboard`).
2. Di menu vertikal paling kiri, klik ikon **SQL Editor** (`>_`).
3. Klik tombol **+ New query**.
4. Salin (copy) semua kode SQL di bawah ini, tempel (paste) ke editor.
5. Klik tombol **Run** (atau tekan `Ctrl + Enter` / `Cmd + Enter`).
6. Setelah selesai, buka **Table Editor** untuk memastikan tabel `bundles` dan `orders` sudah muncul.
